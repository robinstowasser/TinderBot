...
export default class TinderPage {
  ...
  async start() {
    ...
    console.log(this.options.apiToken)
    this.GL = new GoLogin({
  ...
  async checkAndHandleErrors() {
    ...
    const likeOfOut = await this.checkOutOfLikes()
    if (likeOfOut == 1) {
      throw new OutOfLikesError();
    } else if (likeOfOut == 2) {
  ...
  async checkUpgradeLikeDialog() {
    ...
    try {
      ...
      for (var h3 of allH3Els) {
        ...
        if (h3Text && h3Text.toLowerCase().includes('ppgrade your like!')) {
          const closeButton = this.page.locator('div:has-text("No Thanks")');
          if (closeButton) {
  ...
  async checkGlobalPath() {
    const isConnected = this.browser.isConnected();
    console.log("connecting..........checkglobalpad.......", isConnected);
    let divs = await this.page.$$('h3');
    for (var div of divs) {
        const divText = await div?.innerHTML();
        if (divText && divText.toLowerCase().includes('go global')) {
  ...
  async checkOutOfLikes() {
    ...
    let likesPageOutOfLikes = await this.page.evaluate(() => {
      ...
      if (likesPage && likesPage.innerHTML.includes("your chances")) {
        return 1;
      }
      let h3s = document.querySelectorAll('h3');
      for (var h3 of h3s) {
        const h3Text = h3.innerHTML;
        if (h3Text.toLowerCase().includes('out of like')) {
          return 2;
        }
      }
      return 0;
  ...
  async reloadRecsPage(retries: number = 0) {
    tlog("navigating to Recs");
    this.desiredURL = "https://tinder.com/app/recs";
    await Promise.all([
      this.page.goto(this.desiredURL, { waitUntil: "networkidle", timeout: DEFAULT_TIMEOUT }),
  ...
  async checkViewProfile() {
    ...
    if (likesYouCard1) {
      var boundingBox = await likesYouCard1.boundingBox();
      if (boundingBox) {
        await this.page.mouse.click(boundingBox.x + boundingBox.width / 2, boundingBox.y + boundingBox.height / 2);
        await delay(2000);
        const nextButtons = await this.page.$$('button.bullet');
        ...
        await delay(800);
        await this.page.mouse.click(1924, 19);
        return false;
      }
    }
    return true;
