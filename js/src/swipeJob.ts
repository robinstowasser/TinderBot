...
export class SwipeJob {
  ...
  async Create() {
    ...
    if (this.jobType == "status_check") {
      this.swipes = this.shadowBanSwipeCount + 2;
    }
  ...
  async incrementJobSwipesForRecommend(like: boolean) {
    await this.runQuery(
      `
      update swipe_jobs
  ...
  async handleFailure(e: Error) {
    ...
    const isGlobalPad = await this.tp.checkGlobalPath();
    ...
    try {
      if (e.stack.includes("Profile has been deleted")) {
      ...
      } else if (e instanceof OutOfLikesError || isGlobalPad) {
        ...
        await saveLog(this.jobID, new Date().toString(), e.toString());
        let retryStatusCheck = this.jobType == "status_check" && this.retries < 10;
        let retryJob = this.jobType != "status_check" && this.retries < 10;
        ...
        tlog(e.stack)
        tlog('------------------')
          await this.markJobCompleted("socket_connection_error",false);
          // return exitCode; 
        }
      ...
      await saveLog(this.jobID, new Date().toString(), error.toString());
      tlog("error: error in handle error", error);
    } finally {
  ...
  async runLikesJob() {
    ...
    for (let i = 1; i <= this.swipes; i++) {
      ...
      //   viewProfileCycle = getRandom(3, 7);
      //   await this.tp.checkViewProfile();
      // }
  ...
  async runRecommendedJob() {
    ...
    for (let i = 1; i <= this.swipes; i++) {
      ...
      if (viewProfileCounter === viewProfileCycle) {
        viewProfileCounter = 0;
        viewProfileCycle = getRandom(3, 7);
        await this.tp.clickViewProfile();
      }
      ...
      await this.incrementJobSwipesForRecommend(isLike);
      await delayWithFunction(this.insertMatch.bind(this), await this.getSwipeDelay(), 200);
      await delay(100);
      endSwipesCount = i;
    }
