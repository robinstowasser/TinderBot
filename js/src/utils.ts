...
export const saveLog = async (jobID: number, date: string, error: string) => {
  const txtpath = path.join(screenshotPath.toString(), jobID.toString(), date + ".txt");
  await fspromise.writeFile(txtpath, error);
}
